...
export class GroupCommandProvider implements vscode.Disposable {
  ...
  private updateGroupCommands() {
    ...
    // Get current groups
    configMgr.get('groups').then((value: any) => {
      const groups = Array.isArray(value) ? value.filter((item) => typeof item === 'string') : []
            ...
            async (fileUri?: vscode.Uri) => {
              try {
                ...
                await configMgr.save(
                  'resources',
                  previousResources.concat([{ filePath: newResource, group }] as Array<ItemInSettingsJson>),
                )
